...
def loop(ser):
    ...
    for gif in range(0, max_gif):
        for idx in range(0, num_pic):
            ...
            time.sleep(delay_list[idx]/1000)
    time.sleep(3)
    subprocess.run("ffmpeg -y -framerate 30 -f image2 -i '/home/clem/Projets/perso/gif4000/tmp/Captured%d.png' -vf scale=768x1020 output.gif", shell=True)
...
def init_randoms():
    ...
    for num in range(0, num_pic):
        delay_list.append(random.randrange(100, 2000, 50))
